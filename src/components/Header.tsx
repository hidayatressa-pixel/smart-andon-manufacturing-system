...
import { Tv, PhoneCall, Wrench, Map, BarChart3, Volume2, VolumeX, Maximize, Minimize, Settings, Flame, Layers, History, LogOut, ShieldCheck, Sun, Moon } from "lucide-react";
...
export const Header: React.FC<HeaderProps> = ({ activeTab, setActiveTab, activeCalls, soundConfig, setSoundConfig, currentUser, onLogout, onOpenConfig, theme, setTheme, language, setLanguage }) => {
  ...
  return <header className={`sticky top-0 z-40 select-none transition-colors border-b shadow-sm ${isLight?"bg-white border-slate-200 text-slate-800":"bg-neutral-900 border-neutral-800 text-neutral-100"}`}>
    {lineStopCount>0&&<div className="bg-red-600 text-white px-4 py-1.5 text-xs font-semibold flex items-center gap-2 animate-pulse"><Flame className="w-4 h-4"/><span>{language==="en"?`${lineStopCount} LINE STOP`:`${lineStopCount} LINE STOP`}</span></div>}
    <div className="max-w-7xl mx-auto px-3 sm:px-6 py-2 flex flex-wrap items-center justify-between gap-3">
      ...
      <nav className={`flex items-center p-1 rounded-2xl border text-xs font-medium overflow-x-auto max-w-full ${isLight?"bg-slate-100/90 border-slate-200":"bg-neutral-950 border-neutral-800"}`}>
        {(isAdmin||isLeader)&&<button id="tab-main-board" onClick={()=>setActiveTab("main_board")} className={navClass("main_board")}><Tv className="w-3.5 h-3.5 text-cyan-500"/><span>{t("tabMainBoard")}</span>{totalActiveCalls>0&&<span className="bg-red-600 text-white text-[10px] px-1.5 rounded-full font-bold">{totalActiveCalls}</span>}</button>}
        {(isAdmin||isOperator||currentUser?.role==="manager")&&<button id="tab-operator-call" onClick={()=>setActiveTab("operator_call")} className={navClass("operator_call")}><PhoneCall className="w-3.5 h-3.5 text-amber-500"/><span>{t("tabOperatorCall")}</span></button>}
        {(isAdmin||isLeader)&&<button id="tab-responder-terminal" onClick={()=>setActiveTab("responder_terminal")} className={navClass("responder_terminal")}><Wrench className="w-3.5 h-3.5 text-blue-500"/><span>{t("tabResponderTerminal")}</span>{totalActiveCalls>0&&<span className="bg-amber-500 text-slate-950 text-[10px] px-1.5 rounded-full font-bold">{totalActiveCalls}</span>}</button>}
        {(isAdmin||isLeader)&&<button id="tab-plant-map" onClick={()=>setActiveTab("plant_map")} className={navClass("plant_map")}><Map className="w-3.5 h-3.5 text-emerald-500"/><span>{t("tabPlantMap")}</span></button>}
        {isAdmin&&<button id="tab-admin-dashboard" onClick={()=>setActiveTab("admin_dashboard")} className={navClass("admin_dashboard")}><ShieldCheck className="w-3.5 h-3.5 text-rose-500"/><span>Admin</span></button>}
        {isAdmin&&<button id="tab-master-data" onClick={()=>setActiveTab("master_data")} className={navClass("master_data")}><Layers className="w-3.5 h-3.5 text-cyan-600"/><span>{t("tabMasterData")}</span></button>}
        {(isAdmin||isLeader)&&<button id="tab-activity-logs" onClick={()=>setActiveTab("activity_logs")} className={navClass("activity_logs")}><History className="w-3.5 h-3.5 text-amber-500"/><span>{t("tabActivityLogs")}</span></button>}
        {(isAdmin||isLeader)&&<button id="tab-analytics" onClick={()=>setActiveTab("analytics_reports")} className={navClass("analytics_reports")}><BarChart3 className="w-3.5 h-3.5 text-purple-500"/><span>{t("tabAnalytics")}</span></button>}
      </nav>
      <div className="flex items-center gap-1.5">
        ...
        {isAdmin&&<button id="btn-settings" onClick={onOpenConfig} className="p-1.5 rounded-lg" title={t("settings")}><Settings className="w-4 h-4"/></button>}
